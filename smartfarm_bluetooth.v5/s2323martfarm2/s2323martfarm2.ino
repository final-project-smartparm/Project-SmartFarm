...
void loop() {
  if (bluetoothSerial.available()) {
    ...
    if (value >= 1 && value <= 10) {
      ...
      Soil_moisture_reference = value;
      Serial.println(Soil_moisture_reference);
  ...
  RGB_Color(RGB_LED.Color(255, 255, 255), 10);  // 흰색으로 설정
  ...
  // 온도와 습도에 따른 팬 제어
  if (temp > fMax_temp && fanSpeed == 0){
    fanSpeed = 2;
    FanONOFF(fanSpeed); 
  }
  else if (temp > fMax_temp) {
    FanONOFF(fanSpeed); 
